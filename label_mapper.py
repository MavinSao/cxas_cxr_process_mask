...
def _fallback_colors(n: int):
    import colorsys

    out = []
    for i in range(max(0, n)):
        # Evenly spaced hues, fixed saturation/value.
        r, g, b = colorsys.hsv_to_rgb((i / max(1, n)) % 1.0, 0.65, 0.95)
...
def _build_colors(n: int):
    """
    Best-effort colormap generation.

    - If `colorcet` + matplotlib colormaps are available, use `cc.cm.glasbey_bw_minc_20(i)`.
    - Otherwise, try `cc.palette['glasbey_bw_minc_20']` (hex strings) if present.
    - Otherwise, fall back to a simple HSV palette.
    """
    try:
        import colorcet as cc  # optional dependency
    except Exception:
        return _fallback_colors(n)
    ...
    palette = getattr(cc, "palette", {}).get("glasbey_bw_minc_20")
    if isinstance(palette, (list, tuple)) and len(palette) >= n:
        try:
            return [_hex_to_rgba(c) for c in palette[:n]]
        except Exception:
            pass
...
colors = _build_colors(len(id2label_dict.keys()))
colors_alpha = [(c[0], c[1], c[2], c[3] / 2.0) for c in colors]
